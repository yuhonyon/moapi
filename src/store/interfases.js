          ...
          if (/^@/.test(mockValue)) {
            ...
            if (
              ...
              (item.type === "Number" ||
                item.type === "Float" ||
                item.type === "Integer") &&
              /^[0-9.+-]*$/m.test(mockValue)
            ) {
              mockValue = Number(mockValue);
            }
          }
          if (item.mockType === "Array" && /^\[[\s\S]*\]$/m.test(mockValue)) {
            return parseStrToObj(mockValue);
          } else if (
            item.mockType === "Object" &&
            /^{[\s\S]*}$/m.test(mockValue)
          ) {
            return parseStrToObj(mockValue);
            ...
            return Number(mockValue);
          } else if (
            item.mockType === "Boolean" &&
            /^true|false$/m.test(mockValue)
          ) {
            return parseStrToObj(mockValue);
          } else if (item.mockType === "RegExp") {
            return `$$*RegExp*$$${mockValue}$$*RegExp*$$`;
          } else {
            return mockValue;
          }
        } else {
